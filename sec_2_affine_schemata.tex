...
\begin{beispiel}
	...
	\thref{fig:spec c xy} zusammenfassen. 
\end{beispiel}
...
\begin{proof}
	...
	in $M$ liegt. Analog können wir dies von $(y) + \bmax$ sagen. Damit
	...
	$f^{n+m} \in \bmax \in M$, wodurch man den Widerspruch erhält.
...
\begin{satz}
	...
	Die $A_i$ heißen \emph{Komponenten} von $A$.
\end{satz}
\begin{proof}
	...
	\item[Existenz.]
		...
		\]
		Angenommen $\cal V \neq\emptyset$, so hätte man
		ein inklusionsminimales $A \in \cal V$, denn falls nicht gäbe es
		\[
			A_1 \supsetneq A_2 \supsetneq \ldots
		\]
		mit $A_i \in \cal V$. Da $X$ noethersch, müsste diese Folge
		stationär werden, wodurch man einen Widerspruch erhält.
	...
	\item[Eindeutigkeit.]
		...
		Da $A_1$ irreduzibel können wir \obda $A_1 = A_1 \cap A_1'$ 
...
%ich weiß, das ist unschön, aber nach dem align wird der rand der nächsten
%seite benutzt :-(
...
\begin{definition}[(quasi)-kompakt]
	...
	Ist $X = \cap_{i\in I} U_i$, $U_i$ offen, so existiert eine endliche
...
\begin{definition}[$\cal B$-(Prä)Garbe]
	$\F:D(f) \mapsto A_f$ heißt \emph{$\fr B$-Prägarbe} auf
	$X = \Spec A$, wenn 
	$\F$ eine Prägarbe auf 
	\[
