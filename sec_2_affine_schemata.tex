...
\begin{beispiel}
    ...
    \item $x = (XY-1) \ideal A$ ist ebenfalls weder abgeschlossen noch
      generisch.
	\end{itemize}
  Wir künnen die bisherigen Ergebnisse in \thref{fig:spec c xy} zusammenfassen. 
\end{beispiel}
...
\begin{lemma}
	\label{lemma:pi inv ist homüo auf bild}
	...
	\]
	ein Homüomorphismus auf sein Bild
	\[
...
\begin{definition}[(quasi)-kompakt]
	Ein topologischer Raum $X$ heiüt \emph{quasi-kompakt}, wenn gilt:
	Ist $X = \cap_{i\in I} U_i$, $U_i$ offen, so existiert eine endliche
	Teilmenge $F\subset I$ mit
	$X = \cap_{i\in F} U_i$.
	
	$X$ heiüt \emph{kompakt}, wenn $X$ hausdorffsch und quasi-kompakt ist.
\end{definition}
...
Für den Beweis benütigen wir noch eine Definition.

\begin{definition}[$\cal B$-(Prü)Garbe]
